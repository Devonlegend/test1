    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          <div className={styles.headerIcon}>
            <LayoutGrid size={20} color="var(--mbo-forest)" strokeWidth={1.8} />
          </div>
          <div>
      ...
      {!loading && error && (
        <div className={styles.empty} style={{ gap: 12 }}>
          <AlertCircle size={28} color="var(--red-error)" strokeWidth={1.5} />
          <p style={{ color: "var(--red-error)", fontWeight: 600 }}>{error}</p>
          <button
            onClick={() => { setError(null); setLoading(true); }}
            style={{
              padding: "8px 20px", borderRadius: 8, border: "1px solid var(--border-default)",
              background: "var(--surface-white)", fontSize: 13, cursor: "pointer", color: "var(--ink-secondary)",
            }}
          >
            Try again
          </button>
      ...
      {!loading && !error && isVerified === false && (
        <div className={styles.empty}>
          <div style={{
            ...
            background: "var(--amber-tint-bg)", display: "flex",
            alignItems: "center", justifyContent: "center", marginBottom: 12,
          }}>
            <ShieldAlert size={28} color="#ca8a04" strokeWidth={1.5} />
          </div>
          <p style={{ color: "var(--amber-deep)", fontWeight: 600, fontSize: 15 }}>
            Account pending verification
          </p>
          <p style={{ color: "var(--amber-earth)", fontSize: 13, textAlign: "center", maxWidth: 320 }}>
      ...
      {/* VERIFIED â€” show programmes */}
      {!loading && !error && isVerified === true && (
        <>
          {programmes.length === 0 && (
            <div className={styles.empty}>
              <p style={{ color: "var(--ink-muted)", fontWeight: 600 }}>No programmes available</p>
              <p style={{ color: "var(--ink-disabled)", fontSize: 13 }}>
