...
// ── PAGE ──────────────────────────────────────────────────────────────────────
export default function VerifierSchemeReviewPage() {
  const router = useRouter();
  const params = useParams();
  const schemeId = params.schemeId;

  const [data, setData] = useState(null);     // { scheme, pending_review, unpublished, applications }
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const [publishing, setPublishing] = useState(false);
  const [publishMsg, setPublishMsg] = useState(null);   // { type, text }
  ...
  async function handlePublish() {
    ...
    try {
      ...
      await load(true); // refresh unpublished count
    } catch (err) {
      setPublishMsg({
        type: "error",
  ...
  if (error || !data) {
    return (
      <div className={styles.centerState}>
        ...
        <button className={styles.backBtn} onClick={() => router.push("/verifier/applications")}>
            ...
            <p className={styles.modalText}>
              ...
              {!allReviewed && (
                <span style={{ color: "#f59e0b", display: "block", marginTop: 8 }}>
                  Note: {data.pending_review} application{data.pending_review === 1 ? "" : "s"} still need review — they will not be emailed until approved.
                </span>
              )}
            </p>
